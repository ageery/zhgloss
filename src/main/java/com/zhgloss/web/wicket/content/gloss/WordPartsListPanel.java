...
public class WordPartsListPanel extends Panel {
	...
	public WordPartsListPanel(String id, IModel<SegmentedWord> model, IModel<String> wordLinkModel) {
		super(id, model);
		add(new ListView<>("words", new LambdaModel<>(model, new OptionalFunction<>(SegmentedWord.FUNCTION_WORDS)), 
				item -> item.add(new WordPartsPanel("word", new LambdaModel<>(model, new OptionalFunction<>(SegmentedWord.FUNCTION_TEXT)), item.getModel(), wordLinkModel))));
	}
